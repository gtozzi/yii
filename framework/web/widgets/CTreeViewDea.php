...
/**
 * CTreeViewDea displays a tree view of hierarchical data.
 *
 * It encapsulates the excellent tree view plugin for jQuery
 * ({@link http://bassistance.de/jquery-plugins/jquery-plugin-treeview/}).
 *
 * To use CTreeViewDea, simply sets {@link data} to the data that you want
 * to present and you are there.
 *
 * CTreeViewDea also supports dynamic data loading via AJAX. To do so, set
 ...
 * @since 1.0
 */
class CTreeViewDea extends CWidget
{
	/**
	...
	public function init()
	{
		...
		$cs->registerCoreScript('treeviewdea');
		$options=$this->getClientOptions();
		$options=$options===array()?'{}' : CJavaScript::encode($options);
		$cs->registerScript('Yii.CTreeViewDea#'.$id,"jQuery(\"#{$id}\").treeviewdea($options);");
		if($this->cssFile===null)
			$cs->registerCssFile($cs->getCoreScriptUrl().'/treeview/jquery.treeviewdea.css');
